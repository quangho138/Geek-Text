...
def get_update_review(request, book_id, review_id):
    ...
    try:
        ...
        return Response({"message" : 'This review does not exist for this book.'}, status=status.HTTP_404_NOT_FOUND)
...
def get_avg_rating(request, book_id):
    ...
    avg_rating = Rating.objects.filter(book=book).aggregate(Avg('rating'), Count('rating'))
  
    if avg_rating['rating__avg'] is None:
        return Response({'message': 'This book does not have any ratings made yet.'})
  
    print(avg_rating)
    return Response({'Average Rating' : f'{avg_rating["rating__avg"]:.2f}', 'Number of Ratings' : avg_rating['rating__count']}, status=status.HTTP_200_OK)
